...
async function setupProtectedSession(password) {
    ...
    toastService.showMessage("私密会话已启用。");
}
...
ws.subscribeToMessages(async message => {
    ...
    if (message.type === 'task-error') {
        ...
        toastService.showPersistent(makeToast(message, protectingLabel,protectingLabel + " 进行中： " + message.progressCount));
    } else if (message.type === 'task-succeeded') {
        const toast = makeToast(message, protectingLabel, protectingLabel + " 成功完成");
