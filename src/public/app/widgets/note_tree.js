        ...
        <div class="form-check">
            <label class="form-check-label">
                ...
                <span class="bx bx-info-circle" 
                      title="在父笔记中显示的图像不会显示在导航树中"></span>
            </label>
        </div>
    
        <br/>
    
        <button class="btn btn-sm btn-primary save-tree-settings-button" type="submit">保存 & 应用</button>
    </div>
        ...
        this.$tree.fancytree({
            ...
            dnd5: {
                ...
                dragDrop: async (node, data) => {
                    if ((data.hitMode === 'over' && node.data.noteType === 'search') ||
                        ...
                        await infoDialog.info("不允许将笔记放入此位置。");
            ...
            enhanceTitle: async function (event, data) {
                ...
                if (node.data.noteId !== 'root'
                    ...
                    const unhoistButton = $('<span>&nbsp; (<a class="unhoist-button">忘记</a>)</span>');
                ...
                if (note.type === 'search' && $span.find('.refresh-search-button').length === 0) {
                    const refreshSearchButton = $('<span>&nbsp; <span class="refresh-search-button bx bx-refresh" title="刷新保存的检索结果。"></span></span>');
