...
<div class="search-box">
    ...
    <div class="form-group">
        <div class="input-group">
            <input name="search-text" class="search-text form-control"
                   placeholder="检索文本、标签" autocomplete="off">

            <div class="input-group-append">
                <button class="do-search-button btn btn-sm icon-button bx bx-search" title="检索 (enter)"></button>
            </div>
        </div>
    </div>
    ...
    <div style="display: flex; align-items: center; justify-content: space-evenly; flex-wrap: wrap;">
        <button class="save-search-button btn btn-sm"
            title="这将在活动笔记下，创建新的检索笔记。">
            <span class="bx bx-save"></span> 保存搜索
        </button>

        <button class="close-search-button btn btn-sm">
            <span class="bx bx-x"></span> 关闭搜索
        </button>
    </div>
</div>`;
...
export default class SearchBoxWidget extends BasicWidget {
    ...
    doSearch(searchText) {
        ...
        if (searchText.trim().length === 0) {
            toastService.showMessage("请先输入搜索条件。");
