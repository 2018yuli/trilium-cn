...
export default class RelationMapTypeWidget extends TypeWidget {
    ...
    doRender() {
        ...
        this.$relationMapContainer.on("contextmenu", ".note-box", e => {
            contextMenu.show({
                ...
                y: e.pageY,
                items: [
                    {title: "新标签页中打开", command: "openInNewTab", uiIcon: "empty"},
                    {title: "删除笔记", command: "remove", uiIcon: "trash"},
                    {title: "编辑标题", command: "editTitle", uiIcon: "pencil"},
                ],
        ...
        this.$createChildNote.on('click', async () => {
            ...
            const title = await promptDialog.ask({ message: "输入新笔记的标题",  defaultValue: "新笔记" });
            ...
            toastService.showMessage("单击画布以放置新笔记");
    ...
    async connectionCreatedHandler(info, originalEvent) {
        ...
        connection.bind("contextmenu", (obj, event) => {
            if (connection.getType().includes("link")) {
                ...
                contextMenu.show({
                    x: event.pageX,
                    y: event.pageY,
                    items: [ {title: "删除关系", command: "remove", uiIcon: "trash"} ],
                    selectMenuItemHandler: async ({command}) => {
                        if (command === 'remove') {
                            const confirmDialog = await import('../../dialogs/confirm.js');

                            if (!await confirmDialog.confirm("是否确认删除关系？")) {
                                return;
                            }
        ...
        if (relationExists) {
            ...
            await infoDialog.info("连接名 '" + name + "' 在已经存在。");
    ...
    async createNoteBox(noteId, title, x, y) {
        ...
        const $noteBox = $("<div>")
            ...
            .append($("<div>").addClass("endpoint").attr("title", "从这里开始拖动并将它们放到另一个笔记上。"))
            .css("left", x + "px")
            .css("top", y + "px");
    ...
    async dropNoteOntoRelationMapHandler(ev) {
        ...
        for (const note of notes) {
            ...
            if (exists) {
                toastService.showError(`笔记 "${note.title}" 已在关系图中。`);
